...
class APISwaggerFlowTests(APITestCase):
    """Couvre les cas principaux décrits par la doc: création via admin,
    accès restreint user, gestion docs."""
    ...
    def test_admin_creates_association_and_user_can_access_it(self):
        """Admin crée une association avec création auto d'un compte; le nouveau user
        ne voit que son asso."""
        admin_token = self._auth_as("admin", "testpass123")

        # Création via /api/associations/ avec les champs d'user attendus par
        # perform_create
        payload = {
    ...
    def test_user_cannot_see_other_associations(self):
        ...
        results = (
            admin_list.data["results"]
            if isinstance(admin_list.data, dict)
            else admin_list.data
        )
        ...
        results = (
            user1_list.data["results"]
            if isinstance(user1_list.data, dict)
            else user1_list.data
        )
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0]["nom_association"], "Asso 1")

    def test_user_creates_document_and_admin_can_approve(self):
        """User soumet un document, admin peut l'approuver, user ne peut pas
        l'approuver."""
        self._auth_as("admin", "testpass123")
    ...
    def test_document_visibility_respects_role(self):
        ...
        results = (
            user_docs.data["results"]
            if isinstance(user_docs.data, dict)
            else user_docs.data
        )
        self.assertEqual(
            len(results),
        ...
        results = (
            admin_docs.data["results"]
            if isinstance(admin_docs.data, dict)
            else admin_docs.data
        )
    ...
    def test_filter_documents_by_status(self):
        ...
        for status_value in ("submitted", "approved"):
            file_obj = SimpleUploadedFile(
                f"{status_value}.pdf", b"content", content_type="application/pdf"
            )
